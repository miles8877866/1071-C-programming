...
#define AI_OFB					3
#define AI_CFB					4
	ªs²p °s?¯T ºw padding? ½F²¯­X«P.
#define AI_NO_PADDING			1	//	Padding ??(?¥é? 16©x??¯T Öº¯ò)
#define AI_PKCS_PADDING			2	//	paddingµn? ©x?? ¯òªÍ padding
...
typedef struct{
	...
	DWORD		PadType;					//	³ï?Áù?¯T Padding type
	...
	DWORD		BufLen; 					//	Buffer¯T ãÜÅ{ ©x?? ¯ò
	DWORD		RoundKey[AES_NO_ROUNDKEY];	//	§ã´ÚÀQ ­h¯T DWORD ¯ò
} AES_ALG_INFO;

/*************** Constant (Error Code) ************************************/
	Error Code - ?©²??, §Q¤oÏs §K¥é¯³¨ã ?.
#define CTR_SUCCESS					0
#define CTR_FATAL_ERROR				0x1001
#define CTR_INVALID_USERKEYLEN		0x1002	//	«p?­h¯T ¿ë?°Ú ¦ó§Q¨Ò?.
#define CTR_PAD_CHECK_ERROR			0x1003	//	
#define CTR_DATA_LEN_ERROR			0x1004	//	¤^?¯T ¿ë?°Ú ¦ó§Q¨Ò?.
#define CTR_CIPHER_LEN_ERROR		0x1005	//	Áù??? ³ï?¯T Öº¯ò°Ú °s?.
...
/*************** Prototypes ***********************************************/
	??? ?? AES_ALG_INFO«T mode, padding ?´T ´Ã IV ?? Âc«ó®±­X«P.
void	AES_SetAlgInfo(
		DWORD			ModeID,
		DWORD			PadType,
		BYTE			*IV,
	...
	?¥éµ¥ AES_USER_KEY_LEN©x¨c?¯T «p?­hªÍ §ã´ÚÀQ ­h ?¤v
RET_VAL AES_EncKeySchedule(
		BYTE			*UserKey,		//	??½U «p?­h¨j ?¥é?.
		DWORD			UserKeyLen,
		AES_ALG_INFO	*AlgInfo);		//	Áù¦½?? Round Key°Ú ?Íù¹¹.
RET_VAL AES_DecKeySchedule(
		BYTE			*UserKey,		//	??½U «p?­h¨j ?¥é?.
		DWORD			UserKeyLen,
		AES_ALG_INFO	*AlgInfo);		//	Áù¦½?? Round Key°Ú ?Íù¹¹.

	Init/Update/Final ©}?? Áù?®±.
RET_VAL	AES_EncInit(
		AES_ALG_INFO	*AlgInfo);
