...
unsigned int compress_col[4];  //¥[³tª©À£ÁYcolumn --> 32bits = 4 * 8 bits 
//void Mixcolumn(){
//	    for(int i = 0;i < 4;i++){    
//    	byte a = state[0][i];
//		byte b = state[1][i];
...
int main(){
	int te0[256] = {0}; 
	int e1[4][1] = {{0x02}, {0x01}, {0x01}, {0x03}};
	for (int i=0; i<256; i++){
		for(int j = 0; j < 4; j++){
			compress_col[j] = sbox[i] * e1[j][0];
			printf("%x\n", compress_col[j]);
		} 
		te0[i] = compress_col[0] << 6 + compress_col[1] << 4 + compress_col[2] << 2 +compress_col[3];
	}
